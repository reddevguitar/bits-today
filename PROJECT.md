...
- When a candidate shows roughly 95-100% range-position but the entire intraday range is still very tight (roughly 1.5% or less from low to high), treat it as a tight-range hold rather than a full breakout, and cap such narrow-box names to at most 1 of 5 preferred picks unless turnover is already top-5 among non-major alts.
...
- When major average range-position falls below roughly 20 and top15 non-major positives shrink to 4 or fewer, force the next cycle to prefer 5 all-positive non-major candidates where possible, cap paper deployment around 70%, and reject headline leaders with roughly -10% or worse day-high gap even if turnover remains huge.
...
- When major average range-position stays above roughly 80 and fresh breakouts are 5개 이상 while one mega-liquidity attention alt still holds roughly 70%+ of the day range with better than roughly -2% day-high gap, allow exactly 1 such anchor below 80% range-position, but force the other 4 preferred slots into 80%+ leaders so the basket keeps both liquidity and acceleration.
- When fresh breakouts drop to 0 while major average range-position falls by roughly 25 points or more and top15 low-range count jumps by 4 or more within one cycle, force at least 3 of 5 preferred picks out of the prior breakout basket and into 55-75% survivor/reclaim names; if a new high-turnover candidate also has a same-day unlock or supply event overhang, keep it on the watchlist unless its day-high gap is tighter than roughly -3%.
- When major average range-position rebounds by roughly 30 points or more within one cycle and at least 3 non-major leaders hold 80%+ of the day range with real multi-10B KRW turnover, allow at most 1 lower-range mega-liquidity survivor to remain and force at least 2 of 5 preferred picks to upgrade into those cleaner leaders instead of defending the older 60-70% basket.

- When major average range-position stays above roughly 80 and at least 5 non-major candidates already clear roughly 75% of the day range, do not displace multi-10B KRW liquidity leaders with sub-5B prettier charts unless the lower-liquidity name is clearly superior on both gap and momentum; this prevents unnecessary churn during strong breadth.
- When 4개 이상 current holdings break invalidation or lose their earlier upper-range thesis within one cycle while major average range-position is still roughly 75 or higher but fresh breakouts shrink to 1 or fewer, force the next cycle to abandon the broken basket, rebuild around 5 currently positive high-range leaders where possible, and trim deployment toward roughly 85% instead of defending the old holdings.
- When weak_breadth_warning is true, fresh breakouts are 0, and major average range-position drops by roughly 40 points or more within one cycle, force the next cycle to cap deployment around 70%, keep at most 2 salvage/rebound slots, and fill the other 3 slots with the cleanest all-positive secondary continuation names even if their turnover is smaller.

- When 2개 이상 held salvage/rebound slots break invalidation while 4개 이상 all-positive 70%+ continuation/fresh alternatives with real turnover are available, force an immediate full reset into 5 all-positive names and keep deployment around 70% instead of defending the broken salvage basket.
- When a prior 70% all-positive basket loses fresh breakouts to 0 and at least 2 held beta names slip below roughly 45% of the day range, force the next cycle to replace them with cleaner 60-85% continuation/survivor names even if the newer names have smaller turnover; do not defend the older winners for momentum memory alone.
- When major average range-position rebounds by roughly 50 points or more within one cycle, fresh breakouts recover to 3 or more, and top15 non-major positives rise to 10 or more, force the next cycle to abandon any held sub-30% laggards, upgrade deployment toward roughly 85-90%, and rebuild around at least 3 upper-range leaders plus up to 2 mega-liquidity reclaims instead of defending the old 70% survivor basket.
- When 3개 이상 current holdings actually break invalidation while major average range-position is still above roughly 60, force the next cycle to abandon those broken slots, rebuild around 5 currently positive non-major names where possible, and trim paper deployment toward roughly 85% instead of defending the failed basket.
- When major average range-position rebounds from below roughly 15 to above roughly 85 within one cycle and at least 2 non-major names dominate multi-50B KRW turnover while holding roughly 65-75% of the day range, allow exactly 2 mega-liquidity reclaim slots alongside 3 cleaner 80%+ leaders, and raise paper deployment toward roughly 90% instead of staying stuck in the prior defensive basket.
- When major average range-position rebounds into roughly the 30-45 zone but weak_breadth_warning stays true and fresh breakouts are 1 or fewer, cut paper deployment toward roughly 80%, keep exactly 1 mega-breakout core, and rebuild the other 4 slots from all-positive liquid continuation/survivor names instead of defending the prior 90% basket.
